...
def layout_grid_basis_page():
    ...
    plot = etc.make_grid_plot(buffer)
    return flask.render_template(
        'layout/grid_base_layout_page.html',
        plot_script=plot.get('script'),
        plot_div=plot.get('div'),
        js_resources=plot.get('js_res'),
        css_resources=plot.get('css_res')
...
def layout_grid_forecast_setup_page():
    return flask.render_template(
        "layout/grid_forecast_setup_page.html",
...
def layout_grid_forecast_setup_per_column(column_name: str):
    filepath = data_loader.show_open_dialog(
        title="Выберите модель для столбца " + column_name,
        filetypes=(
            ('Модель анализа', '*.mdl'),
            ('Все файлы', '*.*')
        )
...
def layout_grid_forecast_page():
    ...
    for col in buffer.columns:
        if session['models'].get(col) is not None:
            ...
            new_series.append(
                buffer[col].append(pandas.Series(prediction)).rename(col)
            )
            fig = figure(
            ...
            fig.line(
                ...
                color='green',
                line_width=3,
                legend_label=('Прогноз для ' + col)
            )
            fig.legend.location = 'top_left'
            # fig.grid.click_policy="hide"
            figs.append(fig)

    new_buffer = pandas.concat(
        new_series,
        axis=1,
        keys=[s.name for s in new_series]
    )
    ...
    return flask.render_template(
        "layout/grid_forecast_page.html",
        plot_script=script,
        plot_div=div,
...
def layout_grid_forecast_nnet():
    ...
    script, div = components(grid)
    return flask.render_template(
        "layout/grid_forecast_nnet_page.html",
        plot_script=script,
        plot_div=div,
        js_resources=js_resources,
        css_resources=css_resources
