...
def lod_file():
    ...
    if mode == 'url':
        ...
        return flask.render_template(
            'load_page.html',
            file_path=flask.session['filename'],
            form_text="Файл загружен",
            form_text_mode="text-success",
            file_types=file_types
        )
    if 'filename' in flask.session:
    ...
    return flask.render_template(
        'load_page.html',
        form_text="Не выбран файл",
        form_text_mode="text-danger",
        file_types=file_types
    )
...
def select_file():
    flask.session['filename'] = data_loader.show_open_dialog(
