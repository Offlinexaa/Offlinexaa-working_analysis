...
def arima_regression(
    ...
    pred = model.predict(
        start=len(axis_x),
        end=len(axis_x)+len(axis_x_pred)-1,
        typ='levels'
    )
    fig.line(
        axis_x_pred,
    ...
    for p in range(1):
        for d in range(10):
            for q in range(10):
                model = sm.tsa.arima.ARIMA(data, order=(p, d, q)).fit()
                pred = model.predict(
                    start=len(data.to_list()),
                    end=len(data.to_list()) + 5,
                    typ='levels'
                )
